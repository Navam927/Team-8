    ...
    <div className={`${bgClass} flex`}>
      ...
      <aside className="w-72 hidden md:flex flex-col bg-white/90 text-black shadow-xl h-screen sticky top-0 z-10">
        ...
        <nav className="flex-1 py-8 px-4 space-y-2 text-[1rem] font-medium">
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-indigo-50 group transition font-semibold">
            <span>🏠</span>
            <span className="group-hover:text-indigo-700">Dashboard</span>
          </a>
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-indigo-50 group transition">
            <span>📄</span>
            <span className="group-hover:text-indigo-700">My Documents</span>
          </a>
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-indigo-50 group transition">
            <span>⚙️</span>
            <span className="group-hover:text-indigo-700">Settings</span>
          </a>
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-indigo-50 group transition">
            <span>🚪</span>
        ...
        <div className="p-5">
          <div className="bg-indigo-100 rounded-lg p-4 text-indigo-900 text-sm font-medium">
            <span className="font-bold">Tip:</span> Start a new document and invite your collaborators!
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 p-8 md:p-12 flex flex-col">
        ...
        <p className="text-purple-100/80 mb-10 text-lg font-medium max-w-xl">
          Collaborate in real-time, create stunning documents, and unlock your productivity potential.
        </p>

        {/* Action bar */}
        <div className="flex flex-wrap justify-between items-center mb-7 gap-3">
          <h2 className="text-2xl font-bold text-white drop-shadow">Your Documents</h2>
          ...
          {documents.map((doc) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-bold text-lg text-indigo-900 group-hover:text-indigo-700 truncate">{doc.name}</h3>
              </div>
              ...
              <div className="flex justify-between items-end mt-2 gap-2">
                ...
                <button className="ml-auto group-hover:scale-105 bg-indigo-700 text-white px-4 py-1.5 rounded-lg font-semibold text-sm shadow hover:bg-indigo-800 transition active:scale-95">
                  Open
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur">
          <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full flex flex-col gap-5">
            ...
            <div className="mb-2 text-gray-700 font-medium">
              Document: <span className="font-semibold">{selectedDoc?.name}</span>
            </div>
